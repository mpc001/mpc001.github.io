...
tags: Deep Learning
---
本文列举了在做语音增强的一些基本知识点，包括特征提取、噪声实现和评价指标。
...
$\quad$ sigPower = np.sum(data) / (len(data)*1.0)

$\quad$ noisePower = np.sum(noise) / (len(noise)*1.0)

$\quad$ factor = (sigPower / noisePower ) / (10**(snrTarget / 10.0))

$\quad$ data = (data + noise*np.sqrt(factor)).astype(np.float32)
...
##客观评价标准
###Segmental SNR
用信噪比衡量主观语音质量很差，一种客观的评价标准为分段信噪比，此方法基于帧来实现。
$$
...
###PESQ
PESQ全称Perceptual evaluation of speech quality，即主观语音质量评估，该值与平均评定得分存在很强的相关性，因此常被用作评定语音质量。PESQ通过时域上的对称损伤密度和非对称损伤密度来衡量，在采样率16KHz下的信号，帧损伤被定义为20帧(320ms)的损伤密度的综合值，先取L6范数，再取L2范数。
取L6范数
$$
